...
# ========== QUOTE MANAGER ==========
class QuoteManager:
    """Quote management system for Render deployment"""
    ...
    def load_quotes(self):
        """Load quotes with Render-compatible paths"""
        quotes = []
        
        # Multiple path attempts for Render deployment
        possible_paths = [
            "actions/quotes.json",                    # Standard actions folder
            "/opt/render/project/src/actions/quotes.json",  # Render absolute path
            "quotes.json",                             # Root folder
            os.path.join(os.path.dirname(__file__), "quotes.json")  # Same directory
        ]
        ...
        for path in possible_paths:
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    quotes = json.load(f)
                print(f"✅ Loaded quotes from: {path}")
                break
...
class ActionSendQuote(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        category_map = {
            ...
            "ask_success": "motivation",
            "ask_love": "love",
            "ask_humor": "humor",
            "ask_inspiration": "motivation",
            "ask_wisdom": "life"
        }
